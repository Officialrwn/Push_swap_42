...
/*   Updated: 2022/09/12 21:36:51 by leo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_from_right(t_struct st, int *a, int *b)
{
	t_node	*temp;
	int		count;
	int		temp_a_count;
	...
	min = 0;
	temp_a_count = 0;
	a_prev = st.tail_a->prev->num;
	ft_printf("stack_b prev: %s a prev: %s\n", st.stack_b->prev, st.stack_a->prev);
	while (st.tail_a != st.stack_a->prev)
	{
		count = 0 + temp_a_count;
		temp = st.tail_b;
		ft_printf("{YEL}stack_a loop current a->num: %d{EOC}\n", st.stack_a->num);
		while (temp != st.stack_b)
		{
			ft_printf("stack_b loop current b->num: %d\nstackmin: %d stackmax: %d min: %d count: %d, ", temp->num, st.min, st.max, min, count);
			ft_printf("a_prev: %d a->next: %d ", a_prev, st.stack_a->num);
			if (check_push(a_prev, st.tail_a->num, temp->num, st.max, st.min) && (min > count || min == 0))
			{
				if (count > temp_a_count)
					min = count - temp_a_count;
				else
		...
		a_prev = st.tail_a->prev->prev->num;
		st.tail_a = st.stack_a->prev;
		temp_a_count++;
	}
	ft_printf("target a: %d b: %d\n", *a, *b);
	return (min);
